...
def writeToExcel(jsonfile,excfile):
    ...
    sheet1 = book.add_sheet('History', cell_overwrite_ok=True)  # 创建表
    for testlabel in range(len(jsonData['labell'])):
        sheet1.write(testlabel, 1, jsonData['labell'][testlabel])  # 存入history
        sheet = book.add_sheet(jsonData['labell'][testlabel], cell_overwrite_ok=True)  # 创建表
